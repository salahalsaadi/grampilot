...
export default function Channels() {
  ...
  return (
    <div className="grid md:grid-cols-2 gap-4">
      ...
      <div className="card p-4">
        <div className="font-semibold mb-2">Instagram koppelen (mock)</div>
        <p className="text-sm text-neutral-600">Later verbinden we hier echte Instagram via Meta Login. Voor nu kun je mock-kanalen toevoegen om flows te testen.</p>
        <input value={name} onChange={e=>setName(e.target.value)} className="w-full border rounded-xl px-3 py-2 mt-3"/>
        <button onClick={addMock} className="btn btn-primary mt-3">Mock-kanaal toevoegen</button>
      </div>
    </div>
  )
}
      ...
      <div className="card p-4 md:col-span-2">
        ...
        <p className="text-sm text-neutral-600">Hier verschijnt straks de echte Meta Login. Tot die tijd kun je mock-kanalen gebruiken.</p>
        <div className="mt-3 flex gap-2">
          <button className="btn btn-brand">Login met Facebook (uitgeschakeld)</button>
          <button className="btn">Documentatie</button>
        </div>
      </div>
